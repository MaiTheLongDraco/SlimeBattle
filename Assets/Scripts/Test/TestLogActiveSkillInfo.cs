...
public class TestLogActiveSkillInfo : MonoBehaviour
{
    ...
    {
        for (var i = 0; i < SkillToPickInfoHolders.Count; i++) Destroy(SkillToPickInfoHolders[i].gameObject);
    ...
    {
        for (var i = 0; i < numberOfCreate; i++)
        {
    ...
    public void UpdateRuntimeData(SkillID skillID)
    {
        foreach (var skillData in hardSkillTemplateClone.ListInfo)
